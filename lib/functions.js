...
module.exports = function(knex, formatter, postgisFn) {
  var wrapWKT = formatter.wrapWKT.bind(formatter),
      wrapGeoJSON = formatter.wrapGeoJSON.bind(formatter),
      wrapNumber = formatter.wrapNumber.bind(formatter),
      wrapBoolean = formatter.wrapBoolean.bind(formatter),
      postgis;

  postgis = {

    x: function(geom) {
      return knex.postgisFn('ST_X', wrapWKT(geom));
    },
    ...
    geomFromText: function(geom, srid) {
      ...
      if (typeof srid === 'undefined') {
        ...
        return postgisFn('ST_geomFromText', wrapWKT(geom), srid);
      }
    },

    transform: function(geom, srid) {
      return postgisFn('ST_transform', wrapWKT(geom), srid);
    },

    buffer: function(geom, radius) {
      return postgisFn('ST_Buffer', wrapWKT(geom), radius);
    },
