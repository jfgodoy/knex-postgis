...
module.exports = function(knex, formatter, postgisFn) {
  var wrapWKT = formatter.wrapWKT,
      ...
      wrapInteger = formatter.wrapInteger,
  ...
  postgis = {
    ...
    z: function(geom) {
      return postgisFn('ST_Z', wrapWKT(geom));
    },

    m: function(geom) {
      return postgisFn('ST_M', wrapWKT(geom));
    },
    ...
    transform: function(geom, srid) {
      return postgisFn('ST_transform', wrapWKT(geom), wrapInteger(srid));
    },
    ...
    geographyFromText: function(ewkt) {
      return postgisFn('ST_GeographyFromText', wrapWKT(ewkt));
    },
    ...
    distanceSphere: function(a, b) {
      return postgisFn('ST_DistanceSphere',
        wrapWKT(a),
        wrapWKT(b)
      );
    },
    ...
    boundingBoxIntersection: function(a, b) {
      console.log('deprecated. Use boundingBoxIntersects');
      return knex.raw('? && ?', [wrapWKT(a), wrapWKT(b)]);
    },

    boundingBoxIntersects: function(a, b) {
      return knex.raw('? && ?', [wrapWKT(a), wrapWKT(b)]);
    },

    boundingBoxContained: function(a, b) {
      return knex.raw('? @ ?', [wrapWKT(a), wrapWKT(b)]);
    },

    boundingBoxContains: function(a, b) {
      return knex.raw('? ~ ?', [wrapWKT(a), wrapWKT(b)]);
    },

    setSRID: function(geom, srid) {
      return postgisFn('ST_SetSRID', wrapWKT(geom), wrapNumber(srid));
    },
    multi: function(geom){
      return postgisFn('ST_Multi', wrapWKT(geom));
    },    
  };
